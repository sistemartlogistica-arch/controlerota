  ...
  const handleCreateRota = async () => {
    ...
    try {
      const response = await fetch("/api/rotas/create", {
        ...
        body: JSON.stringify({
          origem: newRotaOrigem,
          destino: newRotaDestino,
        }),
      });
        ...
        <div className="modal">
          <div className="modal-content">
            ...
            <div className="modal-actions">
              <button
                onClick={handleEditRota}
                disabled={!editRotaOrigem || !editRotaDestino || loading}
                className="btn-primary"
              >
...
function VanManagement() {
  ...
  const loadVans = async () => {
    try {
      const endpoint = showInactiveVans
        ? "/api/vans/list-inactive"
        : "/api/vans/list";
  ...
  return (
    <div>
      ...
      <div className="vans-list" style={{ padding: "20px", paddingTop: "0" }}>
        ...
        >
          <h3>
            Vans {showInactiveVans ? "Desativadas" : "Ativas"} (
          ...
          vans.map((van: any) => (
            <div key={van.id} className="van-item">
              <div className="van-info">
                ...
                {van.ativa === false && (
                  <span className="van-status" style={{ color: "red" }}>
                    DESATIVADA
                  </span>
                )}
              </div>
          ...
          <div className="modal-content">
            ...
            <div className="modal-actions">
              <button
                ...
                disabled={!editVanKm || !editVanPlaca || loading}
                className="btn-primary"
              >
                {loading ? "Salvando..." : "Salvar"}
              </button>
...
export default function Admin() {
  ...
  const [newUserJornada, setNewUserJornada] = useState([
  ...
  const loadUsers = async () => {
    try {
      ...
      const usersData = snapshot.docs.map((doc: any) => {
        ...
        const userEmail =
          emailData[userData.uid] || userData.email || "Email não encontrado";
        return {
          id: doc.id,
          ...userData,
          email: userEmail,
  ...
  useEffect(() => {
    const fetchVans = async () => {
      try {
        const res = await fetch("/api/vans/list");
        if (!res.ok) throw new Error("Erro ao buscar vans");
    ...
    const fetchRotas = async () => {
      try {
        const res = await fetch("/api/rotas/list");
  ...
  const getFilteredData = () => {
    ...
    filteredRecords.forEach((record) => {
      ...
      data.push([
        ...
        record.abertura?.dataHora
          ? new Date(record.abertura.dataHora).toLocaleString("pt-BR")
          : "",
        userTipo === "copiloto"
          ? "-"
          : record.fechamento?.kmFinal || "Em aberto",
        record.fechamento?.dataHora
          ? new Date(record.fechamento.dataHora).toLocaleString("pt-BR")
          : "Em aberto",
    ...
    const selectedUser = recordFilters.selectedUser
      ? users.find((u) => u.uid === recordFilters.selectedUser)
      : null;
  ...
  const exportCSV = (): void => {
    ...
    if (recordFilters.selectedUser) {
      const selectedUser = users.find(
        (u) => u.uid === recordFilters.selectedUser
      );
      data.push([""]);
      data.push([""]);
      ...
      data.push([
        `Período: ${
          recordFilters.startDate
            ? new Date(
                recordFilters.startDate + "T12:00:00"
              ).toLocaleDateString("pt-BR")
            : "Todas"
        } até ${
          recordFilters.endDate
            ? new Date(recordFilters.endDate + "T12:00:00").toLocaleDateString(
                "pt-BR"
              )
            : "Todas"
        }`,
  ...
  const getJornadasData = () => {
    const filteredRecords = records.filter((record: any) => {
      if (
        ...
        record.userId !== jornadasFilters.selectedUser
      ) {
        return false;
      }
      ...
      if (jornadasFilters.startDate || jornadasFilters.endDate) {
        const recordDate = new Date(record.abertura?.dataHora);
        if (
          jornadasFilters.startDate &&
          recordDate < new Date(jornadasFilters.startDate)
        ) {
          return false;
        }
        if (
          jornadasFilters.endDate &&
          recordDate > new Date(jornadasFilters.endDate + "T23:59:59")
        ) {
          return false;
        }
      }
    ...
    filteredRecords.forEach((record) => {
      const user = users.find((u) => u.uid === record.userId);
      const userJornada = user?.jornada || [
        { entrada: "08:00", saida: "17:00" },
      ];
      const jornadaTexto = userJornada
  ...
  const openJornadaModal = (type: string) => {
    ...
    if (jornadasFilters.selectedUser) {
      const selectedUser = users.find(
        (u) => u.uid === jornadasFilters.selectedUser
      );
  ...
  const getJornadaDataWithHours = () => {
    ...
    filteredRecords.forEach((record) => {
      ...
      const date = record.abertura?.dataHora
        ? new Date(record.abertura.dataHora).toLocaleDateString("pt-BR")
        : "";
      ...
      if (record.abertura?.dataHora) {
        ...
        const saida = record.fechamento?.dataHora
          ? new Date(record.fechamento.dataHora)
          : null;

        groupedData[key].horarios.push({
          entrada,
          saida,
          entradaTexto: entrada.toLocaleTimeString("pt-BR", {
            hour: "2-digit",
            minute: "2-digit",
            hour12: false,
          }),
    ...
    Object.values(groupedData).forEach((group: any) => {
      group.horarios.sort(
        (a: any, b: any) => a.entrada.getTime() - b.entrada.getTime()
      );
    ...
    sortedGroups.forEach((group: any) => {
      ...
      group.horarios.forEach((h: any) => {
        if (h.saida) {
          totalMinutos +=
            (h.saida.getTime() - h.entrada.getTime()) / (1000 * 60);
        }
      });

      const horasTrabalhadas = Math.floor(totalMinutos / 60);
      const minutosTrabalhados = Math.floor(totalMinutos % 60);
      const horasTrabalhadasTexto = `${horasTrabalhadas
        .toString()
        .padStart(2, "0")}:${minutosTrabalhados.toString().padStart(2, "0")}`;
      ...
      const jornadaNormalTexto = `${jornadaNormalHoras
        .toString()
        .padStart(2, "0")}:${jornadaNormalMin.toString().padStart(2, "0")}`;
      ...
      const diferencaTexto = `${sinal}${diferencaHoras
        .toString()
        .padStart(2, "0")}:${diferencaMin.toString().padStart(2, "0")}`;
  ...
  const exportJornadasCSV = (): void => {
    ...
    if (jornadasFilters.selectedUser) {
      ...
      data.push([
        `Período: ${
          jornadasFilters.startDate
            ? new Date(jornadasFilters.startDate).toLocaleDateString("pt-BR")
            : "Todas"
        } até ${
          jornadasFilters.endDate
            ? new Date(jornadasFilters.endDate).toLocaleDateString("pt-BR")
            : "Todas"
        }`,
      ]);
  ...
  const exportJornadasPDF = (): void => {
    ...
    data.slice(1).forEach((row: any, index: number) => {
      if (row.some((cell: any) => cell && cell.toString().trim())) {
        ...
        y += 6;
        doc.text(
          `Trabalhadas: ${row[4]} | Normal: ${row[5]} | Diferença: ${row[6]}`,
          14,
          y
        );
        y += 10;
  ...
  const getFilteredRecords = (): any[] => {
    return records.filter((record: any) => {
      if (
        ...
        record.userId !== chartFilters.selectedUser
      ) {
        return false;
      }
      ...
      if (chartFilters.startDate || chartFilters.endDate) {
        ...
        if (
          ...
          recordDate < new Date(chartFilters.startDate + "T00:00:00-03:00")
        ) {
          return false;
        }
        if (
          chartFilters.endDate &&
          recordDate > new Date(chartFilters.endDate + "T23:59:59-03:00")
        ) {
          return false;
        }
      }
  ...
  const getUserStats = (): any[] => {
    ...
    if (motoristaRecords.length > 0) {
      stats.push({
        ...
        percentage:
          totalRecords > 0 ? (motoristaRecords.length / totalRecords) * 100 : 0,
      });
    }
    ...
    if (copilotoRecords.length > 0) {
      stats.push({
        ...
        percentage:
          totalRecords > 0 ? (copilotoRecords.length / totalRecords) * 100 : 0,
      });
    }
  ...
  const generatePieSlices = (): any[] => {
    ...
    return userStats.map((stat: any, index: number) => {
      ...
      const endAngle =
        ((cumulativePercentage + stat.percentage) / 100) * 360 - 90;
      ...
      <div className="pie-legend-container">
        {userStats.map((stat: any, index: number) => (
          <div key={stat.email} className="pie-legend-item">
            ...
            ></div>
            <span>
              {stat.email}: {stat.count} ({stat.percentage.toFixed(1)}%)
            </span>
          </div>
        ))}
      </div>
  ...
  const generateVanMotoristaChart = (): any => {
    ...
    filteredRecords.forEach((record) => {
      ...
      if (record.fechamento?.kmFinal && record.abertura?.kmInicial) {
        vanStats[key].kmTotal +=
          record.fechamento.kmFinal - record.abertura.kmInicial;
      }
    });

    const statsArray = Object.values(vanStats).sort(
      (a: any, b: any) => b.kmTotal - a.kmTotal
    );
    const maxKm = Math.max(
      ...(statsArray as any[]).map((s: any) => s.kmTotal),
      1
    );
        ...
        {(statsArray as any[]).map((stat: any, index: number) => (
          <div key={`${stat.vanPlaca}-${stat.userName}`} className="bar-item">
            <div className="bar-label">
              ...
              <br />
              <small>
                {formatUserType(stat.userTipo)} • {stat.registros} registros
              </small>
            </div>
  ...
  const getLast7DaysStats = () => {
    ...
    const dayStats = last7Days.map((day) => {
      const dayRecords = filteredRecords.filter((record: any) => {
        const user = users.find((u) => u.uid === record.userId);
        if (!user || user.tipo === "copiloto" || !record.fechamento?.kmFinal)
          return false;
        ...
        {dayStats.map((day, index) => {
          ...
          return (
            <div key={day.fullDate} className="pie-legend-item">
              ...
              ></div>
              <span>
                {day.day}: {day.totalKm} km ({percentage.toFixed(1)}%)
              </span>
            </div>
          );
        })}
  ...
  const generateWeekdayChart = (): any => {
    ...
    const weekdayStats = weekdays.map((weekday, index) => {
      const motoristaKm = filteredRecords
        .filter((record: any) => {
          const user = users.find((u) => u.uid === record.userId);
          if (!user || user.tipo === "copiloto" || !record.fechamento?.kmFinal)
            return false;
      ...
      const mentoraFechamentos = filteredRecords.filter((record: any) => {
        const user = users.find((u) => u.uid === record.userId);
        if (!user || user.tipo !== "copiloto" || !record.fechamento)
          return false;
            ...
            >
              <div className="bar-wrapper">
                ...
                <span className="bar-value">
                  Motoristas: {stat.motoristaKm} km
                </span>
              </div>
              <div className="bar-wrapper">
                <div
                  ...
                  style={{
                    width: `${
                      ((stat.mentoraFechamentos * 10) / maxValue) * 100
                    }%`,
                    background: "linear-gradient(90deg, #28a745, #1e7e34)",
                  }}
                ></div>
                <span className="bar-value">
                  Monitoras: {stat.mentoraFechamentos} fechamentos
                </span>
              </div>
            </div>
          </div>
  ...
  const generateMonitoraChart = (): any => {
    ...
    const statsArray = Object.values(monitoraStats).sort(
      (a: any, b: any) => b.registros - a.registros
    );
    const maxRegistros = Math.max(
      ...(statsArray as any[]).map((s: any) => s.registros),
      1
    );
        ...
        {(statsArray as any[]).map((stat: any, index: number) => (
          <div key={`${stat.vanPlaca}-${stat.userName}`} className="bar-item">
            <div className="bar-label">
              ...
              <br />
              <small>
                {stat.userTipo} • {stat.registros} registros
              </small>
            </div>
            <div className="bar-wrapper">
              <div
  ...
  return (
    <div className="admin-container">
              ...
              {newUserJornada.map((horario, index) => (
                ...
                >
                  <input
                    ...
                    onChange={(e) =>
                      updateJornadaHorario(index, "entrada", e.target.value)
                    }
                    className="input"
                    style={{ flex: 1 }}
                  />
                  ...
                  <input
                    ...
                    value={horario.saida}
                    onChange={(e) =>
                      updateJornadaHorario(index, "saida", e.target.value)
                    }
                    className="input"
                    style={{ flex: 1 }}
                  />
                  ...
                  <div key={user.id} className="user-item">
                    <div className="user-info">
                      ...
                      <div className="user-badges">
                        <span className={`badge ${user.perfil}`}>
                          {user.perfil}
                        </span>
                        {user.perfil === "user" && (
                          <span className={`badge tipo-${user.tipo}`}>
                            ...
                            {(() => {
                              ...
                              user.jornada.forEach((j: any) => {
                                if (j?.entrada && j?.saida) {
                                  const [entradaH, entradaM] = j.entrada
                                    .split(":")
                                    .map(Number);
                                  const [saidaH, saidaM] = j.saida
                                    .split(":")
                                    .map(Number);
                                  const entradaMin = entradaH * 60 + entradaM;
                                  const saidaMin = saidaH * 60 + saidaM;
        ...
        {editingUserJornada && (
          <div className="modal">
            <div className="modal-content">
              <h3>
                Alterar jornada -{" "}
                {editingUserJornada.nome || editingUserJornada.email}
              </h3>
                ...
                {newJornada.map((horario, index) => (
                  ...
                  >
                    ...
                    {newJornada.length > 1 && (
                      <button
                        onClick={() => {
                          const updated = newJornada.filter(
                            (_, i) => i !== index
                          );
                          setNewJornada(updated);
                        }}
                                ...
                                onClick={() => {
                                  ...
                                  setEditRecordData({
                                    kmInicial:
                                      ...
                                      "",
                                    kmFinal:
                                      record.fechamento?.kmFinal?.toString() ||
                                      "",
                                    dataAbertura: formatDateForInput(
                                      record.abertura?.dataHora
                                    ),
                                    dataFechamento: formatDateForInput(
                                      record.fechamento?.dataHora
                                    ),
                                    diarioBordo:
                                      record.fechamento?.diarioBordo || "",
              ...
              <div style={{ marginBottom: "15px" }}>
                ...
                <div style={{ display: "flex", gap: "10px", marginTop: "5px" }}>
                  <input
                    ...
                    onChange={(e) => {
                      const time =
                        editRecordData.dataAbertura.split("T")[1] || "00:00";
                      setEditRecordData((prev) => ({
                        ...prev,
                        dataAbertura: `${e.target.value}T${time}`,
                      }));
                    }}
                    className="input"
                    style={{ flex: 1 }}
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const date =
                        editRecordData.dataAbertura.split("T")[0] ||
                        new Date().toISOString().split("T")[0];
                      setEditRecordData((prev) => ({
                        ...prev,
                        dataAbertura: `${date}T${e.target.value}`,
                      }));
                    }}
                    className="input"
                    style={{ flex: 1 }}
              ...
              <div style={{ marginBottom: "15px" }}>
                ...
                <div style={{ display: "flex", gap: "10px", marginTop: "5px" }}>
                  <input
                    ...
                    onChange={(e) => {
                      const time =
                        editRecordData.dataFechamento.split("T")[1] || "00:00";
                      setEditRecordData((prev) => ({
                        ...prev,
                        dataFechamento: `${e.target.value}T${time}`,
                      }));
                    }}
                    className="input"
                    style={{ flex: 1 }}
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const date =
                        editRecordData.dataFechamento.split("T")[0] ||
                        new Date().toISOString().split("T")[0];
                      setEditRecordData((prev) => ({
                        ...prev,
                        dataFechamento: `${date}T${e.target.value}`,
                      }));
                    }}
                    className="input"
                    style={{ flex: 1 }}
      ...
      <section className="admin-section">
        <div
          className="section-header"
          onClick={() => toggleSection("jornadas")}
        >
          <h2>Relatório de Jornadas</h2>
          <div className="section-actions" onClick={(e) => e.stopPropagation()}>
            <button
              onClick={() => openJornadaModal("csv")}
              className="btn-secondary"
            >
              CSV
            </button>
            <button
              onClick={() => openJornadaModal("excel")}
              className="btn-secondary"
            >
              Excel
            </button>
            <button
              onClick={() => openJornadaModal("pdf")}
              className="btn-secondary"
            >
              PDF
            </button>
            <span className="toggle-icon">
              {expandedSections.jornadas ? "−" : "+"}
            </span>
          </div>
        </div>
                    ...
                    .map((record: any) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <br />
                          Van: {record.placa?.toUpperCase() || "N/A"}
                          <br />
                          Rota:{" "}
                          {record.origem && record.destino
                            ? `${record.origem} → ${record.destino}`
                          ...
                          Abertura:{" "}
                          {record.abertura?.dataHora
                            ? new Date(record.abertura.dataHora).toLocaleString(
                                "pt-BR"
                              )
                            : "N/A"}
                        </div>
            ...
            <div className="chart-filters">
              ...
              >
                ...
                {users.map((user: any) => (
                  <option key={user.uid} value={user.uid}>
                    {user.nome || user.email} -{" "}
                    {formatUserType(user.tipo || "motorista")}
                  </option>
                ))}
              </select>
              <button
            ...
            <div className="charts-container">
              <div className="chart-item">
                ...
                <div className="pie-chart-wrapper">
                  ...
                  <div className="pie-legend">{generateLast7DaysLegend()}</div>
                </div>
              </div>

              <div className="chart-item">
                <h3>KM por Dia da Semana - Motoristas vs Monitoras (Barras)</h3>
                <div className="bar-chart">{generateWeekdayChart()}</div>
              </div>
              ...
              <div className="chart-item">
                ...
                <div className="bar-chart">{generateMonitoraChart()}</div>
              </div>
            </div>
          </div>
        )}
